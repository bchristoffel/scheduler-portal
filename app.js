...
// — Helpers —
// Short: 'MMM DD yy', Full: 'MMM DD yyyy'
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  generateBtn.addEventListener('click', () => {
    ...
    document.getElementById('generateEmails').disabled = scheduleData.length === 0;
  });
...
// 4) Pagination & Email Drafts
function renderEmailPage() {
  const emailPreview = document.getElementById('emailPreview'),
        sendAllBtn = document.getElementById('sendAll');
  emailPreview.innerHTML = '';
  const total = scheduleData.length,
        pages = Math.ceil(total / emailsPerPage),
        startIdx = (emailPage - 1) * emailsPerPage,
        pageData = scheduleData.slice(startIdx, startIdx + emailsPerPage);

  pageData.forEach(r => {
    const email = r[selectedHeaders[0]],
          name  = r[selectedHeaders[1]],
          subject = 'Schedule';

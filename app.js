...
function formatDateShort(d) {
  ...
  return `${m[d.getUTCMonth()]} ${String(d.getUTCDate()).padStart(
    2,
    "0"
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  generateEmails.addEventListener("click", () => {
    ...
    renderEmailPage();
    document
      .querySelector('.tablinks[data-tab="emails"]')
      .click();
  });
...
function onFileLoad(
  ...
  reader.onload = (evt) => {
    ...
    dateRow = (arr[hi - 1] || []).map((c) => {
      ...
      return isNaN(d)
        ? String(c).trim()
        : formatDateShort(new Date(d.toUTCString()));
    });
    headerRow = arr[hi] || [];
    rawRows = arr.slice(hi + 1);
    previewContainer.innerHTML =
      "<p>Select Week Start & click Generate Weekly Preview.</p>";
...
function onGeneratePreview(
  ...
  if (!val) return alert("Pick a Week Start date");
  ...
  if (startIdx < 0)
    return alert(`Date ${labelsShort[0]} not found`);
  const dateIndices = Array.from({ length: 5 }, (_, i) => startIdx + i).filter(
    (i) => i >= 0 && i < dateRow.length
  );
  ...
  if (teamIdx < 0 || emailIdx < 0 || empIdx < 0)
    return alert("Missing Team/Email/Employee");
  ...
  if ((copyBtn = document.getElementById("copyAll")))
...
function renderEmailPage() {
  ...
  const pageData = scheduleData.slice(
    startIdx,
    startIdx + emailsPerPage
  );
  ...
  pageData.forEach((r) => {
    ...
    // Build table header + weekdays spelled out
    ...
    tbl += `</tr><tr><th></th>`;
    selectedHeaders.slice(2).forEach((h) => {
      const dayName = new Date(h).toLocaleDateString("en-US", {
        weekday: "long",
      });
    ...
    selectedHeaders.slice(2).forEach((h) => {
      const v = r[h];
      tbl += `<td style="border:1px solid #ddd;padding:6px;">${
        v || ""
      }</td>`;
    });
    tbl += `</tr>
...
function onSendAll() {
  if (
    !confirm(`Send all ${scheduleData.length} emails now?`)
  )
    return;

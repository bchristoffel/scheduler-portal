...
function onFileLoad(e) {
  ...
  reader.onload = evt => {
    ...
    if (!ws) { alert('Schedule tab not found.'); return; }
...
// 2. Generate preview for WeeklyTemplate
function onGeneratePreview() {
  const startVal = weekStartInput.value;
  if (!startVal) { alert('Please select a Week Start date.'); return; }
  ...
  if (teamIdx<0||emailIdx<0||empIdx<0) { alert('Missing key columns.'); return; }
  ...
  if (!scheduleData.length) { previewContainer.textContent = 'No matching rows.'; return; }
...
function onCopyAll() {
  ...
  range.selectNode(previewContainer.querySelector('table'));
